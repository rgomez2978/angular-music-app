...
export class DeezerService {
  ...
  getArtistAlbumns() {
    return this.getAPI('search?q=metallica').pipe(map((resp: any) => resp.data));
  }
  ...
  getArtistTrack() {
    return this.getAPI('playlist/1068075307').pipe( map( (resp: any) =>  resp.tracks.data));
  }

  // Obtiene los comentarios hacia un artista de un artista
  getArtistComments(id: string) {
    return this.getAPI(`artist/${id}/comments`).pipe( map( (resp: any) =>  resp.data ));
  }
